...
export function createLeverageTools(bridge: WorldBridge): WorldTool[] {
  ...
  const focusObjects = tool('focus_objects', 'Focus the camera on objects', 'Pan and zoom so the given objects fill the view. Not a history commit. Use before explaining or editing something the learner cannot see; use spotlight_objects to ring them first. emphasis chooses how much of the surroundings stays in shot.', schema({
    ...
    anchor: { type: 'string', enum: ['cursor', 'center'], description: '"cursor" holds the point under the pointer still so the shot pushes in where you are looking. Default "center".' },
  }, ['ids']), false, async (input) => {
    const args = values(input, ['ids', 'emphasis', 'anchor']); if (!Array.isArray(args.ids) || !args.ids.length || !args.ids.every((id) => typeof id === 'string' && id)) throw new Error('ids must be a non-empty array of object id strings, e.g. ["graph-1"].')
    ...
    const anchor = args.anchor === undefined ? 'center' : args.anchor
    if (anchor !== 'cursor' && anchor !== 'center') throw new Error('anchor must be "cursor" or "center".')
    ...
    const outcome = await bridge.focusObjects(args.ids as string[], emphasis, anchor)
    return outcome.ok ? { ...outcome, changedIds: outcome.changedIds ?? [], data: { ...(outcome.data ?? {}), ids: args.ids, emphasis, anchor, viewport: bridge.getWorld().viewport } } : outcome
  })
