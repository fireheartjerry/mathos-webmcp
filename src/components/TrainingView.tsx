...
export default function TrainingView({ object, world, run }: Props) {
  ...
  return (
    <section className="training-view" onPointerDown={(event) => { if (event.button !== 2) event.stopPropagation() }}>
      ...
      <div className="training-footnote">central finite differences · deterministic backtracking · {attention ? 'weights linked to Attention' : 'standalone model state'}</div>
    </section>
  )
}
