...
| Video demo, under 3 minutes, public on YouTube, with audio | **BUILT — needs uploading.** [`docs/video/second-try-demo.mp4`](video/second-try-demo.mp4): 1920×1080, H.264 + AAC 48 kHz stereo, **2:44.1** (164.1s, against a 180s limit), 6.0MB, `faststart`. Audio measured, not assumed: **−15.98 LUFS** integrated, **−1.43 dBTP** true peak. The audio was originally written at 96 kHz, which is legal AAC and a needless risk on an upload; it was resampled with the video stream copied, so the picture is bit-identical and the frame count (4920) and duration are unchanged. Assembled by the Remotion composition in `video/` from a real screencast of the production build driven through its own tools, plus a beat-aligned narration. Under the 3:00 limit. What remains is the **YouTube upload**. Swapping the synthetic voice for a human one needs seven replacement WAVs and no code change. |
