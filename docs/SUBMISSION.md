...
| Video demo, under 3 minutes, public on YouTube, with audio | **BUILT — needs uploading.** [`docs/video/second-try-demo.mp4`](video/second-try-demo.mp4): **2560×1440**, H.264 CRF 17 + AAC 48 kHz stereo, **2:51.2** (171.2s against a 180s limit), **−16.0 LUFS** and **−1.5 dBTP** measured. The page is captured at **2560×1600** (a 1280×800 viewport at `deviceScaleFactor: 2`) so everything downscales into the frame rather than being enlarged. Narrated by **Kokoro-82M** running locally, open weights, no API key. The script follows ASD-STE100: no sentence over 25 words, no em-dash, no semicolon, active voice. **The product fills the frame and a camera moves over it**, positioned from element bounding boxes measured at capture time (`video/public/beats.json`) rather than hand-placed. **The screencast is the current build** and everything in it went through `executeTool`. **No third-party trademarks, logos or music**: the picture is this product only, captured page-side with no browser chrome, and the audio is narration alone. What remains is the **YouTube upload**. |
