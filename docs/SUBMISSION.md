...
| Video demo, under 3 minutes, public on YouTube, with audio | **BUILT — needs uploading.** [`docs/video/second-try-demo.mp4`](video/second-try-demo.mp4): 1920×1080, H.264 + AAC 48 kHz stereo, **2:44.1** (164.1s, against a 180s limit), 6.2MB, `faststart`. **The screencast was re-recorded 2026-08-30 against the current build**, so what the video shows is what the repository contains — including the platform list, which used to repeat one sentence under all seven rows and now states it once. Audio measured, not assumed: **−16.00 LUFS** integrated, **−1.50 dBTP** true peak, hit exactly by a two-pass `loudnorm` after the re-render came back at −20.34. The audio was originally written at 96 kHz, which is legal AAC and a needless risk on an upload; it was resampled with the video stream copied, so the picture is bit-identical and the frame count (4920) and duration are unchanged. Assembled by the Remotion composition in `video/` from a real screencast of the production build driven through its own tools, plus a beat-aligned narration. Under the 3:00 limit, and re-rendered 2026-08-30 to fix a layout collision: the beat marker was positioned independently at 44px from the top while the screencast began at 56px, so a long label ran across the product — *"Agents may write. We withdrew the refusal."* did exactly that, over the beat carrying the thesis. Marker, screencast and caption are now three bands that cannot overlap. Frame count (4920) and duration unchanged. **No third-party trademarks, logos or music**: the picture is this product only, captured page-side with no browser chrome, and the audio is narration alone. What remains is the **YouTube upload**. Swapping the synthetic voice for a human one needs seven replacement WAVs and no code change. |
...
| Verified on the artifact that ships | **DONE, and re-verified 2026-08-30 after the last change.** Everything above had only been checked against the dev server. Against the **production build** (`vinext start`): 18 tools register, the judged journey passes **20/20** (on a different generated family from the previous run — `shared-path` rather than `trig-chain`), the accessibility sweep finds **zero** problems on all three pages with worst contrast 5.02:1 — it previously reported the composer's submit button at 4.17:1, which two pixel samples disproved (90.7% #333333 fill, #ffffff glyphs, **12.63:1**), and the check now resolves colour from the cascade rather than trusting `getComputedStyle` alone, every one of the 12 network requests returns 200, a fresh navigation logs **zero** console errors or warnings, all six tool groups sit in the first viewport at 390–1280px with no horizontal scroll, and the landing page and the 404 clear the same widths with **0** overflowing elements and **0** hit targets under 32px (`scripts/layout-check.mjs`, which is new — `responsive.mjs` asserts scratchpad-specific things and could never be pointed at those two pages), and `get_platform` returns its seven verdicts identically on two consecutive runs leaving `getTools()` unchanged. |
...
| Survives a dark-mode visitor | **DONE.** The palette is black and white by design and its contrast is measured; a browser force-inverting an undeclared page would discard those measurements. `color-scheme: light` is now declared at `:root`. Verified on a machine that genuinely reports `prefers-color-scheme: dark` — the page renders as designed, white ground and black ink. |
| Presents itself when the link is shared | **DONE.** The shipped `<meta name="description">` still ended *"...and the learner owns every edit"* — a description of the write refusal this page withdrew, quoted by every search result, link preview and agent page summary. Rewritten to the current thesis, and the page now carries a full Open Graph and Twitter card pointing at `/og.png` (1200×630, the real marked line and the tool console, served 200 from the build). `src/metadata.test.ts` fails if the description reasserts the withdrawn claim, if the card image is missing, or if the declared dimensions do not match the file. |
| Loads fast enough to judge | **DONE, re-measured 2026-08-30 on the current build.** Production build, cache disabled: first contentful paint **84ms**, load **317ms**, **11** requests (was 588ms / 589ms / 12 before the last round of changes). The client chunk is 2.9MB raw because it carries the computer algebra engine — but that figure is a local artifact: `vinext start` sends no `Content-Encoding`, and the same chunk measures **811KB gzip / 645KB brotli**, which is what Cloudflare serves. `scripts/perf.mjs`. |
