...
| Video demo, under 3 minutes, public on YouTube, with audio | **BUILT — needs uploading.** [`docs/video/second-try-demo.mp4`](video/second-try-demo.mp4): 1920×1080, H.264 + AAC 48 kHz stereo, **2:51.8** (171.8s against a 180s limit), 15MB, `faststart`, **−16.01 LUFS** and **−1.50 dBTP** measured. Narrated by `en-US-AndrewMultilingualNeural` through Edge's neural voices, which need no API key. The script follows ASD-STE100: no sentence over 25 words, no em-dash, no semicolon, active voice. The composition opens on a title card, numbers each of the seven beats, holds one short line rather than subtitling the narration, shows a seven-segment progress rule, and closes on a card with the repository and the licence. **The screencast is the current build** and everything in it went through `executeTool`. **No third-party trademarks, logos or music**: the picture is this product only, captured page-side with no browser chrome, and the audio is narration alone. What remains is the **YouTube upload**. |
